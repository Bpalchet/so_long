...
/*   Updated: 2025/04/09 17:54:37 by bpalchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_map_file(t_game *game, char *map_file)
{
	...
	game->map = (char **)ft_calloc(line_count, sizeof(char *));
	if (!game->map)
		exit_with_error("Error: Failed to allocate memory for map");
...
void	parse_map(t_game *game, char *map_file)
{
	...
	{
		free_map(game);
		cleanup_and_exit(game, "Error: Invalid map");
	}
}
...
int	validate_map(t_game *game)
{
	...
	{
		cleanup_and_exit(game, "Error: map with invalid path");
		return (0);
	}
	return (1);
}
