...
/*   Updated: 2025/04/03 19:30:03 by bpalchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize_window(t_game *game)
{
	...
	game->win = mlx_new_window(game->mlx, game->map_width * TILE_SIZE,
			game->map_height * TILE_SIZE, "So Long Bonus");
	if (!game->win)
	{
		free(game->mlx);
...
void	count_collectibles(t_game *game)
{
	...
	while (i < game->map_height)
	{
		...
		{
			j = 0;
			while (j < game->map_width && game->map[i][j] != '\0')
			{
				if (game->map[i][j] == 'C')
					game->collectibles++;
				j++;
...
void	load_textures(t_game *game)
{
	...
	game->textures.enemy1 = mlx_xpm_file_to_image(game->mlx,
			"textures/enemy1.xpm", &width, &height);
	game->textures.enemy2 = mlx_xpm_file_to_image(game->mlx,
			"textures/enemy2.xpm", &width, &height);
	if (!game->textures.enemy1 || !game->textures.enemy2)
		exit_with_error("Error: Failed to load enemy textures");
